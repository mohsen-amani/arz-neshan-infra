...
for (const serviceName of serviceNames) {
  assert(compose.services?.[serviceName], `Missing Compose service: ${serviceName}`);
}

assert(
  compose.services.api.image === compose.services.migration.image &&
    compose.services.api.image === compose.services['financial-migration'].image,
  'The API and both migration jobs must use the same immutable image.',
);
...
for (const [serviceName, service] of Object.entries(compose.services)) {
  assert(!service.build, `${serviceName} must pull an image instead of building source.`);
  assert(!service.container_name, `${serviceName} must not set container_name.`);
  assert(
...
assert(compose.volumes?.attachments_data, 'Missing persistent attachments_data volume.');
assert(compose.volumes?.clamav_database, 'Missing persistent clamav_database volume.');
...
for (const serviceName of releaseServices) {
  ...
  if (state.source_run_number === 0) {
    ...
    assert(
      /^[0-9a-f]{40}$/.test(state.source_sha) && state.tag === `sha-${state.source_sha}`,
      `Release metadata for ${serviceName} is not an immutable full-SHA release.`,
    );
  }

  const imageServices = serviceName === 'frontends' ? ['web', 'platform'] : ['api'];
  for (const imageService of imageServices) {
    const expectedImage =
      `regi.mohsenamani.com/arz-neshan-infra/${imageService}:${state.tag}`;
    assert(
